...
echo "white,black,date,result" > "$ROOT_DIR/$SLUG/pairings.csv"
...
NR == 1 { next }  # Skip header
{
    ...
    # Skip if not a ladder tournament or if missing required fields
    if (format != "ladder" || white == "" || black == "") next

    # Skip entries with malformed names (containing "/" or incomplete)
    if (white ~ /\// || white ~ /^[[:space:]]*$/ || black ~ /\// || black ~ /^[[:space:]]*$/) next
